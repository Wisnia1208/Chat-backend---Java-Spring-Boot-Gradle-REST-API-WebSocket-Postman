...
getMessageButton.onclick = function () {
  fetch("/api/messages/" + messageUserId.value, { method: 'GET', credentials: 'same-origin' })
    .then(handleResponse)
    .then(showMessage)
    .catch(function (err) {
      showMessage(err.message);
...
sendMessageButton.onclick = function () {
  fetch('/api/messages', {
    ...
    body: JSON.stringify({ messageText: messageText.value, messageReceiverId: messageUserId.value })
  })
    .then(handleResponse)
    .then(showMessage)
    .catch(function (err) {
